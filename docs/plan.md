# Alumbra Comares — Delivery Plan

## Index

| Milestone | Title | Status | PR Scope |
|-----------|-------|--------|----------|
| [M1](m1.md) | Project foundations and style review | pending | Style doc, plan files, repo structure |
| [M2](m2.md) | JSON content model | pending | Content files for all sections |
| [M3](m3.md) | HTML/CSS scaffold — structure and design system | pending | Base HTML, CSS tokens, fonts, layout shell |
| [M4](m4.md) | Hero, nav, and footer | pending | Top and bottom of page |
| [M5](m5.md) | Philosophy, Why Alumbra, and pillars sections | pending | Brand story sections |
| [M6](m6.md) | Meet the midwives section | pending | Three profile cards |
| [M7](m7.md) | Services section | pending | Service cards with placeholders |
| [M8](m8.md) | Events section | pending | Event cards with placeholder data |
| [M9](m9.md) | Contact section and form | pending | Form + contact channels, Formspree |
| [M10](m10.md) | Testimonials and brochure placeholders | pending | Placeholder blocks |
| [M11](m11.md) | Privacy and cookie pages | pending | Lightweight draft legal pages |
| [M12](m12.md) | GitHub Pages deployment | pending | Deploy config, final checks |

## Design Reference

All build milestones (M3–M12) must follow [`docs/style.md`](style.md).

## Source Material

All content milestones (M2–M10) draw from [`assets/reference-docs/reference-content-in-text.md`](../assets/reference-docs/reference-content-in-text.md).

## Principles

- Build mobile-first
- Placeholders are valid where real content is missing
- Spanish at launch, content model must support future English and Valenciano
- Each milestone is one atomic PR — reviewable independently
- Do not harden implementation decisions that block future iterations
